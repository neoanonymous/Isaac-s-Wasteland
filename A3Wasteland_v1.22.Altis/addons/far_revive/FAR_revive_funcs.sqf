	...
	if (_EH == "FAR_deathMessage") then
	{
		...
		if (alive _unit) then
		{
			switch (true) do
			{
				...
				default {
				//systemChat format ["%1 was injured by %2", toString _unitName, toString _killerName]; //Disable injured text but keep gut text. MeanY
				// systemChat format ["%1 was injured by %2", toString _unitName, toString _killerName]; //Disable injured text but keep gut text. MeanY
				};
			};
		};
...
{	
	...
	if (CAN_PERFORM) then
	{
		...
		// [player, "gutCount", 1] call fn_addScore;  //Not set up in database yet
