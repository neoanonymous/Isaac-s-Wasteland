/*
Script edit for Exile by Otto for Exile - http://www.exilemod.com/profile/20458-otto/
...
// Debug can be seen by all who connect to server leave off when live  
ZBNOAM = true;      // true/false removes ambient life from map   
ZBEAISTAT = true; // Set to false if AI scripts are not working correctly it will then Cache Vehicles and objects only 
 
 
 /* ------ DON'T EDIT BELOW THIS LINE ------ */
 diag_log format ["AI Cache Loaded - ", time]; 
...
call compileFinal preprocessFileLineNumbers "cache\zbe_functions.sqf";
	...
	while {true} do {
		...
		zbe_players = (switchableUnits + playableUnits);
		{
			...
			if (!_disable && !(_x in zbe_cachedGroups)) then {
				 ...
				 [zbe_aiCacheDist, _x, zbe_minFrameRate, zbe_debug] execFSM "cache\zbe_aiCaching.fsm";
				};
		} forEach allGroups;
	};
};
		...
		_assetscar = zbe_centerPOS nearEntities ["LandVehicle", zbe_mapside];
		{
			if !(_x in zbe_cached_cars) then {
				zbe_cached_cars = zbe_cached_cars + [_x];
					[_x, zbe_vehicleCacheDistCar] execFSM "cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetscar;
		_assetsair = zbe_centerPOS nearEntities ["Air", zbe_mapside];
		{
			if !(_x in zbe_cached_air) then {
				zbe_cached_air = zbe_cached_air + [_x];
				    [_x, zbe_vehicleCacheDistAir] execFSM "cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetsair;
		_assetsboat = zbe_centerPOS nearEntities ["Ship", zbe_mapside];
		{
			if !(_x in zbe_cached_boat) then {
				zbe_cached_boat = zbe_cached_boat + [_x];
					[_x, zbe_vehicleCacheDistBoat] execFSM "cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetsboat;
...
[] spawn {
	if (zbe_debug) then {
			while {true} do {
                ...
                <t color='#A1A4AD' align='left'>Obj draw distance:</t><t color='#FFFFFF' align='right'>%8</t><br/><br/>
				<t color='#FFFFFF'>Updated for Exile after 1.52Patch by Otto</t><br/>
