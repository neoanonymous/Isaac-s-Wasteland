...
{
	["Engine still running. JEEEZZZZ. Learn to follow simple instructions. Deal CANCELLED!", 5] call mf_notify_client;
	mutexScriptInProgress = false;
};
...
if (!isNil "_price") then 
{
	...
	{	
		...
		sleep 5;
		["Smoko time, back in an hour...", 5] call mf_notify_client;
		sleep 5;
		...
		sleep 5;
		["Repairing, WTF happened to this thing, Learn to drive...", 5] call mf_notify_client;
		_vehicle setDamage 0;
		sleep 5;
		["Rearming, You miss miss a lot...", 5] call mf_notify_client;
		...
		playSound "FD_Finish_F";
		_text = format ["%1 Is now yours to lock and it saves too. It is also fully repaired and refuelled. Oh wait, where did these left over bolts go....", _vehClass];
