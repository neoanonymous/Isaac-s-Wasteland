...
[MF_ITEMS_CANNED_FOOD, "Canned Food", {65 call mf_items_survival_eat}, "Land_BakedBeans_F","client\icons\cannedfood.paa", 5] call mf_inventory_create;
[MF_ITEMS_WATER, "Water Bottle", {65 call mf_items_survival_drink}, "Land_BottlePlastic_V2_F","client\icons\waterbottle.paa", 5] call mf_inventory_create;
...
_code =
{
	...
	if (count _objs > 0) then
	{
		...
		_obj = _objs select 0;
		_obj setVariable ["food", (_obj getVariable ["food", 0]) - 0, true];
...
_code =
{
	...
	if (count _objs > 0) then
	{
		...
		_obj = _objs select 0;
		_obj setVariable ["water", (_obj getVariable ["water", 0]) - 0, true];
