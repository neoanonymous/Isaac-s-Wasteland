...
// if you have anjky doubts and/or questions about the mission find us at a3wasteland.com
...
A3W_maxSpawnBeacons = 5;
// Time settings
A3W_startHour = 9;                // In-game hour at mission start (0 to 23) - time is saved and restored between server restarts if A3W_timeSaving = 1
...
// Timers
APOC_coolDownTimer = 800;         // APOC Air Drop Cool Down Timer
BoS_coolDownTimer = 2460;           // Baselocker hacking timer
Safe_coolDownTimer = 2460;          // Safe hacking timer
...
// Spawn settings
A3W_townSpawnCooldown = 6*60;      // Number of seconds to wait between each spawn on a specific town (0 = disabled)
A3W_spawnBeaconCooldown = 6*60;    // Number of seconds to wait between each use of a specific spawn beacon (0 = disabled)
...
A3W_combatAbortDelay = 120;         // If playerSaving = 1, delay in seconds for which to disable abort and respawn buttons after firing or being shot (0 = none)
...
A3W_serverSavingInterval = 2*60;   // Interval in seconds between automatic vehicle & object saves; should be kept at 1 min for profileNamespace and iniDB, while for extDB it can be relaxed to 3-5 mins
...
A3W_payrollAmount = 1050;           // Amount of money rewarded per territory on each payroll
